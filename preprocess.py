...
new_label = ['bua', 'tlk', 'hok', 'sko', 'tur']  # 不需要空格
...
def makeData(srcFile, tgtFile, srcDicts, tgtDicts, sort=False, char=False):
    ...
    return dataset(src, tgt, raw_src, raw_tgt, from_train)
...
def make_embedding(extra_word):
    ...
    for i, line in enumerate(lines_test):
        label = line.strip()  # 之前还split了，愚蠢啊 和dict文件的格式不一样的
        try:
...
def main():
    ...
    if opt.share:
        ...
        dicts['src'] = initVocabulary('source', './trg.txt', opt.src_vocab,
                                      opt.src_vocab_size)
