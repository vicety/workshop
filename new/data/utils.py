...
def eval_metrics(reference, candidate, label_dict, log_path):
    ...
    def make_label(l, label_dict):
        ...
        # fixedit: 原来是 indices = [label_dict.get(label.strip().lower(), 0) for label in l] 但是显然空格不能去掉……
        indices = [label_dict.get(label.lower(), 0) for label in l]  # 分成单个标签
        result[indices] = 1
        return result

    # reference(真实句子)[batch, len(not same)] candidate(预测句子) label_dict
