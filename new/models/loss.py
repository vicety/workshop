...
new_labels = ['bua', 'tlk', 'zah', 'sko', 'buk', '<unk>']
...
def margin_criterion(tgt_vocab_size, use_cuda, config, tgt_vocab=None):
    def hinge_margin_loss(scores, target, from_known=None, sen_len=None):
        '''
        :param scores: [time*batch, vocab],
        :param target: [maxlen*batch]
        :return: loss:float
        ...
        if use_cuda:
            ...
            t_b, cls = scores.shape
            for i in range(t_b):
                for label in [40, 41, 42, 43, 44, 1]:
                    if label == target[i]:
                        assert loss[i] == 0
            '''
            
            # hint: 瞎改
            # sen_len = int(scores.shape[0] / config.batch_size)
            if sen_len:
                target_2dim = target.view(sen_len, -1)
                loss = loss.view(sen_len, -1)
                mask_through_time = torch.zeros(sen_len).float().cuda()
                for i in range(loss.shape[1]):
                    certain_bach_labels = target_2dim[:, i]
                    for label_id in certain_bach_labels:
                        if label_id in [1, 56, 57, 58, 59, 60]:
                            loss[:, i] *= mask_through_time
                loss = loss.view(-1)
...
def cross_entropy_loss(hidden_outputs, decoder, targets, criterion, config, model, from_known, sim_score=0):
    ...
    if config.score == 'hinge_margin_loss' or config.score == 'softmax':
        ...
        from_known = from_known.t().expand(sentence_len, -1).cuda().long().contiguous().view(-1)  # epoch结尾
        softmax_loss = (criterion['softmax'](scores['softmax'],
                                        from_known)\
                                        + sim_score) * config['softmax_linear_lr_mul']   # ([time*batch, vocab], [maxlen * batch])
        # softmax_loss = torch.Tensor(0).cuda() * softmax_loss
        margin_loss = criterion['margin'](scores['margin'], targets.view(-1),
                                          from_known, sentence_len) + sim_score
        print('softmax_loss: {} || margin_loss: {}'.format(softmax_loss, margin_loss))
