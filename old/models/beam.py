...
class Beam(object):
    def __init__(self, size, n_best=1, cuda=True):
        self.size = size  # size is a scalar
    ...
    def advance(self, wordLk, attnOut):
        ...
        * `attnOut`- attention at the last step
        ...
        if len(self.prevKs) > 0:
            # [K, words] + [size, 1] ----expand to---> [K, words]
            # so i think size == K
            beamLk = wordLk + self.scores.unsqueeze(1).expand_as(wordLk)

            # Don't let EOS have children.
            for i in range(self.nextYs[-1].size(0)):
                # nextY should have shape of [now_len, K]
                # nextYs[-1] should have shape [K]
                if self.nextYs[-1][i] == self._eos:
                    beamLk[i] = -1e20
        else:
            beamLk = wordLk[0]  # since there's only one prev, K == 1
        ...
        prevK = bestScoresId / numWords  # from which father
